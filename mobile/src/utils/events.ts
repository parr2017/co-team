...
export function describeEvent(type: string, p: Record<string, any> = {}): EventView {
  switch (type) {
    ...
    case 'task_plan_failed':
      return { text: `需求评估/计划生成失败${p.error ? ` — ${CLIP(p.error, 100)}` : ''}`, level: 'error', category: 'task', noisy: false };
    ...
    // ---- 协作会话（convo）----
    case 'convo_status':
      return { text: `协作会话：${({ idle: '空闲', running: '执行中', waiting_approval: '待审批', waiting_ask: '待回答' } as Record<string, string>)[p.status] || p.status || ''}`, level: 'info', category: 'system', noisy: true };
    case 'convo_message':
