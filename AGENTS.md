...
- **目录监狱**：任务命令中出现的越界绝对路径 / `..` 逃逸一律拒绝执行（所有权限级别生效，`full` 仅指目录内完全控制）；产物与日志一律写项目目录内。唯一例外：协作会话的 `unrestricted`（无边界）会话级权限——命令与**读取**解除监狱；越界的**常见开发命令**（node/java/npm/bash/python/git 等，链式每段首词均需命中且解释器内联代码不含写/删特征）直接执行并记审计日志，**其他越界命令仍转人工审批**；**写/删文件工具仍锁项目目录内**；该级别不进全局配置、任务 `execution_policy` 与群组讨论，自指任务永不适用。
