...
- **目录监狱**：任务命令中出现的越界绝对路径 / `..` 逃逸一律拒绝执行（所有权限级别生效，`full` 仅指目录内完全控制）；产物与日志一律写项目目录内。唯一例外：`unrestricted`（无边界）权限——命令与**读取**解除监狱；越界的**常见开发命令**（node/java/npm/bash/python/git 等，链式每段首词均需命中且解释器内联代码不含写/删特征）直接执行并记审计日志，**其他越界命令仍转人工审批**；**写/删文件工具仍锁项目目录内**。`unrestricted` 自 2026-09-23 起对全局配置、任务 `execution_policy` 与协作会话 **三处同时开放**（此前仅协作会话可用、任务侧静默回退 base.level，是"开了无边界却没生效"的根因）；该权限在群组讨论中永不适用（讨论引擎固定 `full`），自指任务永不适用。
...
- **UI 改动必须考虑移动端**：任何涉及界面（`web/`）的修改，必须同步评估 `mobile/`（Vant）端是否需要对应调整并一并落地；新增事件、卡片、状态展示、对话框等 UI 元素时 web 与 mobile 双端保持一致。只改 web 不看 mobile 视为未完成。
