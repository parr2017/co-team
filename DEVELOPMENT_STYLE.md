...
## 13. dev 模式运行注意事项（可用性测试教训）

- `npm run dev` 使用 `tsx watch`，**server/src 下任何文件变更都会立即重启进程**，进行中的 HTTP 请求（包括耗时数分钟的任务创建/规划）会直接丢失且任务不落盘。并行开发或长任务执行期间，请使用无 watch 方式启动（`npx tsx server/src/index.ts` 或构建后 `node server/dist/index.js`）。
- LLM 调用默认超时 10 分钟（`COTEAM_LLM_TIMEOUT_MS` 可覆盖），OpenAI SDK 内部重试已关闭——模型故障切换由模型池统一负责，不要在客户端层再叠加重试。
- 模型池对连续失败 ≥3 次的模型按指数冷却（1→15 分钟）摘除，`/api/status` 的 `fail_count`/`cooldown_ms` 可观测。
