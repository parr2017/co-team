...
function openDeliverable(entry: JournalEntry) {
  ...
  if (md) {
    deliverableView.value = { title: entry.node_name || entry.node_id, markdown: md, nodeId: entry.node_id, defects: entry.meta?.defects || [] };
  } else {
    // journal cap may have evicted the body — fetch on demand
    void api.getDeliverable(props.taskId, entry.node_id).then((d) => {
      deliverableView.value = { title: d.node_name || entry.node_id, markdown: d.markdown, nodeId: entry.node_id, defects: d.defects || [] };
    }).catch(() => {});
  }
}
